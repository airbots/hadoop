...
public class JobInProgress {
  ...
  JobInProgress(JobTracker jobtracker, final JobConf default_conf, 
      JobInfo jobInfo, int rCount, Credentials ts,int PreNodeNumRemoteMap,int PreNodeNumLocalMap)
  ...
  throws IOException, KillInterruptedException, UnknownHostException {
    ...
    this.queueMetrics.addWaitingReduces(getJobID(), numReduceTasks);
     //each Map ---each taskProgress   ------yjw
 ...
 //**********************Algorithm 1,2,3******************************************//
  ...
  final int M=numTaskTrackers; //Number of Node
  int T= numMapTasks;          //NUmber of Map tasks

  //calculate the array K[i] ----slots of each node //Number of slots of node i
  int jj=0;
  int [] k=new int [M];
  //for (final TaskTrackerStatus  tts : activeTaskTrackers()){???
    //final int mapSlots = tts.getMaxMapSlots();???
  final int [] mapSlots={4,4,8,8};
    //for (int i=0;i<M,i++){
    //if (jj<M){
    // if (jj<activeTaskTrackers.length) {
     // k[jj]=mapSlots;
    //}
   // jj++;
 // }
  ...
  double [][] AvailTime=new double [M];

  int [] AssumedNodeNumLocalMap=new int [M];
  int [] AssumedNodeNumRemoteMap=new int [M];
  int [] PreNodeNumLocalMap=new int [M];
  int [] PreNodeNumRemoteMap=new int [M];
  int [][] AssumedNumLocalMap=new int [M][];
  int [][] AssumedNumRemoteMap=new int [M][];
  int [][] PreNumLocalMap=new int [M][];
  int [][] PreNumRemoteMap=new int [M][];
  double [][] MFT=new double [M][];
  double [] minMFT=new double [M];
  double [] SFT=new double [M];
  double [] D=new double [M];
  double [] DperB=new double [M];
  double minSFT;
  int [] index_minMFT=new int [M];
  ...
  double[][] TlocalMap={{4.3,4.3,4.3,4.3},{7.8,7.8,7.8,7.8},{7.6,7.6,7.6,7.6,7.6,7.6,7.6,7.6},
          {16.34,16.34,16.34,16.34,16.34,16.34,16.34,16.34}};
  double[][] TremoteMap={{6.3,6.3,6.3,6.3},{7.8,7.8,7.8,7.8},{9.4,9.4,9.4,9.4,9.4,9.4,9.4,9.4},
          {17.29,17.29,17.29,17.29,17.29,17.29,17.29,17.29}};
  //double[][] TlocalMap={{3.9,3.9,3.9,3.9},{3.9,3.9,3.9,3.9},{3.9,3.9,3.9,3.9,3.9,3.9,3.9,3.9},{3.9,3.9,3.9,3.9,3.9,3.9,3.9,3.9}};
  //double[][] TremoteMap={{3.9,3.9,3.9,3.9},{3.9,3.9,3.9,3.9},{3.9,3.9,3.9,3.9,3.9,3.9,3.9,3.9},{3.9,3.9,3.9,3.9,3.9,3.9,3.9,3.9}};
  //node to block map
  HashMap<String, HashSet<String>> nodeToBlock;
  int[][] Z={{0,1,2,6,9,10,11,14,17,18,20,21,25,26,27,28,31},
          {2,3,4,7,8,10,13,14,16,17,18,19,21,22,25,29,31},
          {0,1,4,5,6,7,9,12,15,16,23,24,27,28,30},
          {3,5,8,11,12,13,15,19,20,22,23,24,26,29,30}};//Block Z={Z1,Z2,Z3,Z4}
  //block to node map
  int[][] Taskonnode={{0,2},{0,2},{0,1},{1,3},{1,2},{2,3},{0,2},{1,2},{1,3},{0,2},{0,1},{0,3},{2,3},{1,3},{0,1},{2,3},{1,2},{0,1},{0,1},
          {1,3},{0,3},{0,1},{1,3},{2,3},{2,3},{0,1},{0,3},{0,2},{0,2},{1,3},{2,3},{0,1}};
  int[] MaxnumMaps={17,17,15,15};//Maxnum Maptask of node(i)
